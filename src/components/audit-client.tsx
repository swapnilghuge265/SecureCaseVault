...
export default function AuditClient({ logs }: { logs: LogRow[] }) {
  ...
  const actions = useMemo(
    () =>
      [...new Set(logs.map((l) => l.action))].sort((a, b) =>
        a.localeCompare(b),
      ),
    [logs],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter((l) => {
      if (userFilter !== "all" && l.username !== userFilter) {
        return false;
      }
            ...
            <table className="w-full min-w-[1100px]">
              ...
              <tbody className="divide-y divide-line">
                {pageItems.map((l) => (
                  ...
                  >
                    <td
                      className="td whitespace-nowrap text-mut"
                    ...
                    <td className="td whitespace-nowrap">
                      {getDeviceLabel(l.userAgent)}
                    </td>

                    <td className="td max-w-[160px]">
                      <p
                        className="truncate font-mono text-[11px] text-mut-2"
                        title={l.userAgent ?? ""}
                      >
                        {l.userAgent ?? "—"}
                      </p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line px-4 py-2.5">
          <p className="text-xs text-mut-2">
            ...
            <span className="hidden sm:inline">
              {" "}
              · latest 1000 retained in view · records are append-only
            </span>
          </p>
