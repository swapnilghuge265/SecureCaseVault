...
export async function POST(req: Request) {
  ...
  let body: { fullName?: string; username?: string; email?: string; password?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (!/^[a-z0-9_]{3,24}$/.test(username))
    return Response.json({ error: "Username must be 3–24 characters: letters, numbers or underscores." }, { status: 400 });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
    return Response.json({ error: "Please enter a valid email address." }, { status: 400 });
  if (password.length < 8 || !/[a-zA-Z]/.test(password) || !/\d/.test(password))
    return Response.json({ error: "Password must be at least 8 characters and include letters and numbers." }, { status: 400 });

  // --- Uniqueness checks
  const [dupUser] = await db.select({ id: users.id }).from(users).where(eq(users.username, username));
  if (dupUser) return Response.json({ error: "That username is already taken." }, { status: 409 });
  const [dupEmail] = await db.select({ id: users.id }).from(users).where(eq(users.email, email));
  if (dupEmail) return Response.json({ error: "An account with that email already exists." }, { status: 409 });

  // --- Create the account
  const passwordHash = await bcrypt.hash(password, 10);
  const [inserted] = await db
    .insert(users)
    .values({ fullName, username, email, passwordHash, role: "viewer" })
    .returning();
  ...
  await logAudit(
    { userId: inserted.id, username, action: "login", detail: "Signed in after registration", ip },
    req,
  );

  // Same adaptive cookie strategy + token fallback as the login route.
  const headers = new Headers({ "Content-Type": "application/json" });
  for (const setCookie of sessionCookieHeaders(req, token)) {
    headers.append("Set-Cookie", setCookie);
  }
  return new Response(JSON.stringify({ ok: true, username, sessionToken: token }), { status: 200, headers });
}
